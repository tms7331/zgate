...
use alloy_primitives::{address, Address, Signature as AlloySignature};
...
fn main() {
    ...
    let sig_hex: String = env::read();
    let message: String = env::read();
    let signer: Address = env::read();

    // let (encoded_verifying_key, signer, message, signature): (
    //     EncodedPoint,
    //     Address,
    //     Vec<u8>,
    //     Signature,
    ...
    let sig_bytes = hex::decode(sig_hex.clone()).unwrap();
    let signature_secp = Signature::try_from(sig_bytes.as_slice()).unwrap();
